...
| **Karrera emaitzak** | Karrera bat hautatu eta bere sailkapena **editatu**: errenkada bakoitzeko **postua** (`Sailkapena`) eta **puntuak** (`Puntuak`) inline aldatu (`table-update`), edo **🗑** errenkada ezabatu (`table-delete`; PK = `Karrera_ID`+`Txirrindularia_ID`). **🗑 Emaitza guztiak ezabatu** botoiak karrera osoaren `KarreraSailkapena` husten du. Ondoren *Sailkapenak kalkulatu* exekutatu porralari-puntuak eguneratzeko |
...
| **Baliabideak** | Txapelketa bat hautatu eta bere **fitxategiak osatu**: **arauak / startlist / porrak** PDFak (egoera ✓/⚠/✗; dagoen fitxategia aukeratu edo **igo** → `Txapelketak.*_PDF` DBan gordeta, config fallback-arekin), **webgune ofiziala** (itzuli handiak → `Txapelketak.Web_Ofiziala`; klasikak → lotutako lasterketa kanonikoaren `Lasterketak.Web_Ofiziala`, **Lasterketak** atalarekin partekatua) eta **karreren profil-irudiak** (karrera bakoitza ✓/✗; falta bada, irudia aukeratu edo igo → `Karrerak.Profil_Irudia`). Egoera bezero aldean `api/files` + config bidez; idazketa `table-update`. **🔤 Estandarizatu izenak**: botoi bakar batek txapelketaren fitxategiak izen kanonikora **berrizendatzen** ditu diskoan ETA DB erreferentzia (`*_PDF` / `Profil_Irudia`) eguneratzen du (hala webgune publikoak config ukitu gabe funtzionatzen du, DBk gainidazten baitu). PDFak → `Mota Kirola Urtea.pdf` (adib. `Arauak Giro 2023.pdf`; `klasikoak→klasikak`, urtea 4 digitu); profilak → etapa-lasterketak `Etapa{Ordena}.{ext}`, klasikak karreraren izena. **Aurrebista** erakusten da lehenik (item bakoitza: Berrizendatu / Jada ondo / Gatazka / Ezin egotzi / Diskoan falta); gatazkak (helburua lehendik dagoenean) **saltatu** egiten dira, ez dute inoiz gainidazten. Ibilbide-orokorreko irudia (`IbilbideOsoa.jpg`) ez da ukitzen. `POST baliabideak/standardize` (`apply:false` aurrebista, `apply:true` aplikatu). Migrazioak: `db/txapelketa-fitxategiak.sql`, `db/lasterketak.sql` |
| **Lasterketak** | Klasikak **urte artean bateratu**: lasterketa bakoitzak errenkada independentea du urteko (`Karrerak`), eta izenak ez dira egonkorrak urte artean (adib. "Tour des Flandres" 2025 vs "Ronde van Vlaanderen - Tour des Flandres" 2026). Atal honek **lasterketa kanoniko** bat kudeatzen du lasterketa bakoitzeko (`Lasterketak` taula), `Karrerak.Lasterketa_ID`-z lotuta — `PorraEzizenak ↔ Porralariak` ereduaren parekoa. Izen berdinak **automatikoki** lotuta datoz (migrazioaren hazia); izen-aldaerak eskuz bateratu behar dira: **iradokizunak** antzekotasunaren arabera proposatzen dira (`fuzzy_name_score`, ≥60 puntu). Kanoniko bakoitzari **webgune ofiziala** jarri daiteke — behin sartuta, lotutako urte GUZTIETAN balio du (herentzia automatikoa). **🔀 Bateratu**: bi kanoniko elkartu (transakzioan, ezin da desegin); URLa bietako bat mantentzen da. `GET lasterketak`, `POST lasterketak/lotu` / `/askatu` / `/bateratu`. Migrazioa: `db/lasterketak.sql` |
