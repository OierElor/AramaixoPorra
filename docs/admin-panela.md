...
| **Puntu finalak** | **Itzuli handien amaiera** (porralariak). Txapelketa bat hautatu eta porralari bakoitzaren `Ezizena · Generala · Mendia` itsatsi (zutabeak). Tresnak **etapetako puntuak** batzen ditu (auto, `KarreraSailkapena`-tik) eta **totala** kalkulatzen (`etapak + generala + mendia`), postuak esleituz. **Aurreikusi** botoiak taula erakusten du (Pos·Ezizena·**Etapak**·Generala·Mendia·**Totala**) + lotu gabeko ezizenak. **Gorde eta txapelketa itxi**: `TxapelketaEmaitzaPorralariak` idatzi (Puntuak/Generala/Mendikoa/Posizioa) eta txapelketa **itxi** (`Amaituta = 1`, `Porra_Irekita = 0`). Berriz exekutatzea segurua (ez-suntsitzailea). Migrazioa: `db/amaituta.sql` |
