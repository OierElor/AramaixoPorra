...
| **Porra prestatu** | `tresnak/porra-prestatu/` | Porra IREKITA dagoen txapelketetan, porralariak bere **listak** sortzen ditu (Etapa, Mendia, Orokorra...) eta startlista horietan banatzen du (mailaka ere sailka ditzake lista bakoitzean); gero listetatik hainbat **porra zirriborro** osatzen ditu, alderatzeko. **Etapa motak** atalean txapelketako etapak ere sailka daitezke motaka (Sprint/Ihesaldia/Orokorra..., mota bakarra etapako), `Emaitzarik_Ez=1` dutenak kanpo utzita. Dena nabigatzailearen `localStorage`-n gordetzen da (gailu honetan bakarrik, DB-rik ez). Amaitutakoan **Porra bidali**-ra bidaltzen du (`?txap=ID&dortsalak=1,2,...`). |
